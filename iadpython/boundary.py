...
def Add_Top(n, R01, R10, T01, T10, R12, R21, T12, T21, method):
    """
...
def Add_Bottom(n, R01, R10, T01, T10, R12, R21, T12, T21, method):
    """
...
def Add_Slides(R01, R10, T01, T10, R, T, method):
    ...
    reflection and transmission for the slab with boundary conditions are R_30 and  T_03 
    ...
    Returns:
        'R_total', 'T_total'       : R, T for all 3 with top = bottom boundary
    """
    n=method.quad_pts
    R12 = R
    R21 = R
...
def Sp_RT(s, method):
    ...
    The sample is characterized by the 
    ...
    for i in range(n):
        mu_outside=Cos_Snell(s.n_s, angle[i], 1.0)
        if mu_outside != 0: 
            r, t = Sp_mu_RT(s.n_above, s.n_s, s.n_below, s.b_above, s.b, s.b_below, mu_outside)
            uru += twoaw[i] * r
            utu += twoaw[i] * t
    
    Sp_mu_RT(s.n_above, s.n_s, s.n_below, s.b_above, s.b, s.b_below, s.cos_angle, ur1, ut1)
