...
class layer(unittest.TestCase):
    ...
    def test_01_double(self):
        ...
        s = iadpython.start.Slab(a=0.5, b=1, g=0.0, n=1)
        m = iadpython.start.Method(s)
        rr, tt = iadpython.start.init_layer(s, m)
        np.testing.assert_approx_equal(m.b_thinnest, 0.0625)

        b=0.12500
        rr, tt = iadpython.layer.add_layers(rr, tt, rr, rr, tt, tt, m.twonuw)
    ...
    def test_02_double(self):
        ...
        s = iadpython.start.Slab(a=0.5, b=1, g=0.0, n=1)
        m = iadpython.start.Method(s, quad_pts=4)
        rr, tt = iadpython.start.init_layer(s, m)
        np.testing.assert_approx_equal(m.b_thinnest, 0.0625)
        rr, tt = iadpython.layer.double_until(rr, tt, m.b_thinnest, m.b_calc, m.twonuw)
    ...
    def test_03_double(self):
        ...
        s = iadpython.start.Slab(a=0.5, b=1, g=0.9, n=1)
        m = iadpython.start.Method(s, quad_pts=4)
        rr, tt = iadpython.start.init_layer(s, m)
        rr, tt = iadpython.layer.double_until(rr, tt, m.b_thinnest, m.b_calc, m.twonuw)
